        ...
        public override void Up()
        {
            ...
            Sql(@"CREATE PROCEDURE [dbo].[GetWeeklyUsage]	
                ...
                BEGIN
	                ...
	                FROM Usages
                    INNER JOIN Users ON Users.UserId = Usages.UserId
                    LEFT JOIN (SELECT UserId, DATEPART(ww, [Date]) As [Week], SUM(Electricity) As Electricity, SUM(Gas) As Gas FROM [References] WHERE DATEPART(yy, [Date]) = @Year GROUP BY UserId, DATEPART(ww, [Date]), DATEPART(yy, [Date])) R ON Users.UserId = R.UserId AND R.[Week] = DATEPART(ww, [Timestamp])
