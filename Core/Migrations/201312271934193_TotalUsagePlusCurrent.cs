namespace Core.Migrations
{
    ...
    public partial class TotalUsagePlusCurrent : DbMigration
    {
	...
	SELECT DATEPART(dy, GETDATE()) As NumberOfDays, SUM(RS.E1) As E1, SUM(RS.E2) As E2, SUM(RS.ETotal) AS ETotal, SUM(RS.E1Retour) As E1Retour, SUM(RS.E2Retour) As E2Retour, SUM(RS.ERetourTotal) As ERetourTotal,
		...
		MAX(CURR.E1Meter) AS E1Meter, MAX(CURR.E2Meter) AS E2Meter, MAX(CURR.GasMeter) AS GasMeter
		FROM
		(SELECT 
        ...
        ON 1 = 1  
        LEFT JOIN (
			SELECT MAX(E1Current) As E1Meter, MAX(E2Current) As E2Meter, MAX(GasCurrent) As GasMeter
			FROM Usages
			INNER JOIN Users ON Users.UserId = Usages.UserId
